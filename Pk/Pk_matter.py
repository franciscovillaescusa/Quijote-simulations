...
def compute_Pk(snapshot, snapnum, grid, MAS, threads, NCV, pair, 
    ...
    if NCV:  #paired-fixed simulations
        ...
        if not(os.path.exists(fpk)):
            ...
            find_Pk(snapshot, snapnum, grid, MAS, do_RSD, axis, threads, ptype,
                    fpk, save_multipoles)

        # redshift-space
        for axis in [0,1,2]:
            fpk = '%s/%s/NCV_%d_%d/Pk_%s_RS%d_z=%s.txt'\
        ...
        # redshift-space
        for axis in [0,1,2]:
            fpk = '%s/%s/%d/Pk_%s_RS%d_z=%s.txt'%(folder_out,cosmo,i,suffix,axis,z)
            if not(os.path.exists(fpk)):
                do_RSD, save_multipoles = True, True
                find_Pk(snapshot, snapnum, grid, MAS, do_RSD, axis, threads, ptype,
                        fpk, save_multipoles)
...
def find_Pk(snapshot, snapnum, grid, MAS, do_RSD, axis, threads, ptype,
            fpk, save_multipoles):
...
for i in numbers:
    ...
    if cosmo in ['Mnu_p', 'Mnu_pp', 'Mnu_ppp', 'Mnu_p/', 'Mnu_pp/', 'Mnu_ppp/']:
        ...
        compute_Pk(snapshot, snapnum, grid, MAS, threads, NCV, pair,
                   folder_out, cosmo, i, suffix, z, ptype)
