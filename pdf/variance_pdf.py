...
def compute_PDF(snapshot, grid, MAS, threads, NCV, pair, 
    ...
    if NCV:  #paired-fixed simulations
        ...
        # real-space
        fpdf = '%s/%s/NCV_%d_%d/variance_PDF_%s_z=%s.txt'\
        ...
        """
        for axis in [0,1,2]:
            fpdf = '%s/%s/NCV_%d_%d/variance_PDF_%s_RS%d_z=%s.txt'\
        ...
        # real-space
        fpdf = '%s/%s/%d/variance_PDF_%s_z=%s.txt'%(folder_out,cosmo,i,suffix,z)
        if not(os.path.exists(fpdf)):
            do_RSD, axis = False, 0
        ...
        """
        for axis in [0,1,2]:
            fpdf = '%s/%s/%d/variance_PDF_%s_RS%d_z=%s.txt'%(folder_out,cosmo,i,suffix,axis,
                                                         z)
            if not(os.path.exists(fpdf)):
                do_RSD = True
                find_pdf(snapshot, grid, MAS, do_RSD, axis, threads, ptype, 
...
MAS       = 'CIC'
threads   = 1
smoothing = np.array([5.0, 10.0, 15.0])
